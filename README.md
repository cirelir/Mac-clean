...
- 扫描 `~/Library/Caches`、`~/Library/Logs`、`~/Library/Application Support` 和 `~/Library/Developer` 的直接子目录，不扫描用户文档、下载、照片、音乐或项目目录。无法枚举或读取的目录（例如 macOS 保护的 CloudKit 缓存）会被跳过，不会中断整个扫描，也绝不会成为清理候选。
- 系统数据扫描器把 `~/Library/Logs` 中与已安装应用同名的日志目录（超过 7 天且应用未运行）作为绿色可清理项，把 `DiagnosticReports` 崩溃报告作为系统数据绿色项；其他未知日志目录仅报告。
- 开发数据扫描器把 `~/Library/Developer/Xcode/DerivedData` 的构建产物作为绿色可清理项（Xcode 已安装且未运行时）；Developer 顶层目录（CoreSimulator、XCTestDevices 等）列为待确认项供选择清除。
- 只有与已安装且未运行应用匹配的可再生缓存/日志才成为绿色候选项。匹配会实际核对 `/Applications`、`/System/Applications`、`/Library/Input Methods`、`~/Applications` 中真实安装的应用，并容忍命名变体（如 kimi-desktop 匹配 Kimi、com.microsoft.VSCode.ShipIt 匹配 VSCode）。无主的残留数据——`~/Library/Caches` 与 `~/Library/Application Support` 中核对后确认无对应应用的数据目录（例如已卸载应用的残留）——标记为黄色"待确认"并带"推荐清除"标记（理由注明已核对范围、推断已卸载），在详情页勾选后移入废纸篓；`com.apple.*` 系统容器、已知 macOS 系统服务目录（Knowledge、Spotlight 等）和本应用自身数据永不推荐清除；正在运行应用的缓存只报告。
- 用户可以手动扫描和清理绿色候选项，也可以在详情页勾选黄色"待确认"项目后一并清理（移入废纸篓）。左键点击菜单栏图标打开面板；右键（或 Control-点击）菜单栏图标弹出菜单，可直接退出应用。菜单根视图出现时会根据本地最新扫描时间补做错过的每周扫描；自动流程只执行绿色项。
- 应用保证单实例运行：重复启动不会创建第二个进程，只会激活已运行的实例并让新进程退出。
- 每周补扫只在所有扫描器都完成时记录完成时间；任何扫描器失败都会保留失败信息、不清理、不通知，并让同一计划继续处于待补扫状态。手动扫描仍可展示并保存其他扫描器返回的部分结果。
- 每次清理都会在生成计划前重新读取运行应用清单；候选项所属应用若已经启动，或最新清单无法读取，本轮不会执行该候选项，也不会发送自动清理成功摘要。
