...
候选路径先标准化并解析符号链接，再以 `pathComponents` 做组件级包含判断，不能用字符串前缀伪造位于允许根目录内。目标缺失、解析后越出固定允许根、等于任一允许根目录，或命中明确禁止路径时都会被拒绝。生产配置明确禁止 `/`、用户主目录和 `~/Library/Caches` 本身；执行动作只允许清空已验证候选目录的内容并保留候选根目录。

解析后越界的符号链接会在候选验证阶段被拒绝。即使 direct child alias 仍解析在允许根目录内，只有原始 child 的标准化路径与验证后的 canonical 路径完全相同，扫描器才可能声明已安装应用所有权；类型也从 canonical 对象读取。扫描器在异步统计大小前后对 canonical 对象各做一次不跟随链接的完整 fingerprint，并要求完全相同后才允许产生绿色候选。稳定但未知的目录仍以红色证据报告，遍历错误继续作为 scanner failure 传播。执行器打开候选根目录时使用 `O_NOFOLLOW`，遍历子目录时也使用 descriptor-relative `openat(..., O_NOFOLLOW)`。

绿色缓存必须有已安装应用的明确 Bundle ID 所有者，并且该 Bundle ID 不在 `NSWorkspace` 的运行应用集合中。未知所有者、未知发现类型和正在运行应用的缓存都是红色 report-only。清理流程在 planner 前重新读取一次应用清单；若绿色候选项的所有者此时已经运行，该候选项不会进入计划。若最新清单读取失败，整次清理 fail closed。当前实现不扫描项目目录，不根据 access time 推断未使用依赖，也没有 Homebrew、npm、pip、Cargo 或 Xcode 专用扫描器。
